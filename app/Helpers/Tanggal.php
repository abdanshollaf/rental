...
class Tanggal {
	...
	public static function IndoNow($now) {
        $tanggal = substr($now,8,2);
		$bulan = self::get_bulan_full(substr($now,5,2));
    ...
    public static function Indo2($tgl) {
        ...
        $bulan = self::get_bulan_full(substr($tgl,0,2));
...
public static function get_bulan_full($bln)
    ...
    static function terbilang($angka) {
	...
	public static function terbilang_lama($angka) {
		// pastikan kita hanya berususan dengan tipe data numeric
		$angka = (float)$angka;
